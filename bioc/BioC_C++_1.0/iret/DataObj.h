...
class Index {
   public:
      ...
      /** no longer used -- code left just in case
...
class Indx {
   public:
      ...
      void Compose(Indx<Z> *pKnd); //Assumes that the current objects
        ...
        //Identifies a subset by indices
      long Subvalue(Z j); //on success returns value k+1 such that idx[k]=j.
          //Otherwise returns 0.
      Indx<Z> *Subvalue(Indx *jnd); //jnd must represent a subset and the 
          //returned Indx object will give the values k such that idx[k]
          //is among jnd->idx values.
      long Lbound(Z n); //Returns the smallest value j such that
          //n<=idx[j]. If there is no such j it returns ix.
      long Ubound(Z n); //Returns the largest value j such that
          //idx[j]<n. If there is no such j it returns 0

      Indx<Z> *Subinterval(long n,long m); //Makes the index from
         //interval n<=x<m (idx[n] to idx[m-1]).
      Indx<Z> *Subsample(long n,long seed); //Generates a random sample of
         //idx of size n if ix>n, else returns this.
      Indx<Z> *Subsample(long n); //Generates a random sample of
         //idx of size n if ix>n, else returns this.
         //Uses current state of random number generator
...
Indx<Z> *Indx<Z>::cbool_Butnot(Indx<Z> *jnd){
   long i,j,k,w,p,m,flab,bx,sx,*pdx;
   Z *bdx,*sdx,bu,su;
...
template<class Z>
Indx<Z> *Indx<Z>::Subinterval(long n,long m){
   if(m<=n)return(NULL);
   if(n<0)return(NULL);
   if(ix<m)return(NULL);
...
template<class Z>
Indx<Z> *Indx<Z>::Subsample(long n,long seed){
   long i,j,*ptr;
...
template<class Z>
Indx<Z> *Indx<Z>::Subsample(long n){
   long i,j,*ptr;
...
Ordr<Y,Z> *Ordr<Y,Z>::cbool_And(Indx<Y> *jnd){
   ...
   Indx<Y> *pind=this->cbool_And(jnd);
...
Ordr<Y,Z> *Ordr<Y,Z>::cbool_Butnot(Indx<Y> *jnd){
   ...
   Indx<Y> *pind=this->cbool_Butnot(jnd);
   if(pind==NULL)return(NULL);
   if(pind->ix<1)return(NULL);
